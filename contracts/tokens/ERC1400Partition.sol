...
contract ERC1400Partition is IERC1400Partition, ERC1400Raw {
  ...
  function transferByPartition(
    ...
    returns (bytes32)
  {
    return _transferByPartition(partition, _msgSender(), _msgSender(), to, value, data, "", true);
  }
  ...
  function operatorTransferByPartition(
    ...
    returns (bytes32)
  {
    require(_isOperatorForPartition(partition, _msgSender(), from), "A7"); // Transfer Blocked - Identity restriction

    return _transferByPartition(partition, _msgSender(), from, to, value, data, operatorData, true);
  }
   ...
   */
  function setDefaultPartitions(bytes32[] calldata partitions) external onlyRole(0) {
  ...
  function transferWithData(address to, uint256 value, bytes calldata data)
    external
    // checks are being done in the internal function _transferByPartition
  {
    _transferByDefaultPartitions(_msgSender(), _msgSender(), to, value, data, "", true);
  }
  ...
  function transferFromWithData(address from, address to, uint256 value, bytes calldata data, bytes calldata operatorData)
    external
  {
    require(_isOperator(_msgSender(), from), "A7"); // Transfer Blocked - Identity restriction

    _transferByDefaultPartitions(_msgSender(), from, to, value, data, operatorData, true);
  }
