...
contract("Test Whitelist", async (accounts) => {
	...
	it("deploy WhitelistConstraintModule", async () => {
		whitelistConstraintModule = await WhitelistConstraintModule.new(contracts.micoboSecurityToken.address)
	})
	...
	it("cannot edit whitelist whithout being Whitelist_editor", async () => {
		await truffleAssert.fails(whitelistConstraintModule.editWhitelist(accounts[0], true))

		await truffleAssert.fails(whitelistConstraintModule.bulkEditWhitelist(accounts, true))
	})
	...
	it("can edit whitelist when Whitelist_editor", async () => {
		...
		assert.deepEqual(await whitelistConstraintModule.isWhitelisted(accounts[1]), false)
	})
	...
	it("gets correct module name", async () => {
		assert.deepEqual(await whitelistConstraintModule.getModuleName(), Module.WHITELIST)
	})
})
