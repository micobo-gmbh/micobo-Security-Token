...
contract("Test Redeeming", async (accounts) => {
	...
	before(async () => {
		...
		await contracts.micoboSecurityToken.issueByPartition(conf.standardPartition, accounts[3], value, "0x0", {
			from: issuer,
		})
	})
	...
	it("can redeem own tokens only if redeemer", async () => {
		// should fail because is not redeemer
		await truffleAssert.fails(
			contracts.micoboSecurityToken.redeemByPartition(conf.standardPartition, value, "0x0", { from: accounts[3] })
		)
		...
		await truffleAssert.fails(
			contracts.micoboSecurityToken.redeemByPartition(conf.standardPartition, value - 1, "0x0", {
				from: accounts[3],
			}),
		...
		// new balance matches
		assert.deepEqual(
			(await contracts.micoboSecurityToken.balanceOfByPartition(conf.standardPartition, accounts[3])).toNumber(),
			value - value
		)
	})
