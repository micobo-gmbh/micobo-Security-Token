...
contract("Test Token Partition ERC20 Proxy", async (accounts) => {
	...
	it("partition gives me all the correct token information", async () => {
		...
		assert.deepEqual(await contracts.securityTokenPartition.partitionId(), conf.standardPartition)

		assert.deepEqual((await contracts.securityTokenPartition.cap()).toNumber(), conf.standardCap)

		assert.deepEqual(await contracts.securityTokenPartition.name(), conf.name)

		assert.deepEqual(await contracts.securityTokenPartition.symbol(), conf.symbol)

		assert.deepEqual((await contracts.securityTokenPartition.decimals()).toNumber(), 18)

		assert.deepEqual((await contracts.securityTokenPartition.totalSupply()).toNumber(), value * 2)
	})
	...
	it("can transfer using proxy", async () => {
		...
		let balance = (await contracts.securityTokenPartition.balanceOf(accounts[1])).toNumber()
	...
	it("can approve and transferFrom using proxy", async () => {
		...
		await truffleAssert.fails(
			contracts.securityTokenPartition.transferFrom(accounts[1], accounts[0], value, { from: accounts[0] })
		)

		// cannot approve 0 address
		await truffleAssert.fails(
			contracts.securityTokenPartition.approve("0x0000000000000000000000000000000000000000", value, {
				from: accounts[1],
			}),
		...
		assert.deepEqual((await contracts.securityTokenPartition.allowance(accounts[1], accounts[0])).toNumber(), value)
	...
	it("can use partition without proxy contract", async () => {
		...
		let balance = (await contracts.micoboSecurityToken.balanceOfByPartition(somePartition, accounts[0])).toNumber()
