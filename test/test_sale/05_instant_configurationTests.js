...
contract("Test Instant Configuration", async (accounts) => {
	...
	it("cannot add fiat purchase if not admin", async () => {
		truffleAssert.fails(
			sale.addFiatPurchase(tokenAddress, conf.standardPartition, accounts[1], amount, { from: accounts[1] }),
	...
	it("admin can edit currency rates", async () => {
		...
		await sale.editCurrencyRates(tokenAddress, conf.standardPartition, mock.currencyAddress, rate * 2)
